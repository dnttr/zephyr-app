...
void zc_app::chat_area::setup_chat_text_styles()
{
    ...
    container attach_icon_container(attach_button.get_container().get_x() + 18, attach_button.get_container().get_y() + 12);
...
void zc_app::chat_area::update_scrollbar()
{
...
void zc_app::chat_area::rebuild_message_display(bool animate_new_messages)
{
    ...
    for (size_t i = 0; i < conversation_msg_count; ++i)
    {
        ...
        am.message_text->initialize(
            wrapped_content, container(bubble_x + MESSAGE_TEXT_LEFT_RIGHT_PADDING, messages_content_start_y + current_relative_y + MESSAGE_TEXT_TOP_BOTTOM_PADDING), font, message_style
        );
        ...
        am.original_timestamp_y = messages_content_start_y + current_relative_y + bubble_height - TIMESTAMP_HEIGHT_IN_BUBBLE;
...
std::string zc_app::chat_area::format_timestamp(const std::chrono::system_clock::time_point &timestamp)
{
...
void zc_app::chat_area::animate_scroll()
{
...
void zc_app::chat_area::handle_scrollbar_drag(const zcg_mouse_pos_t &pos)
{
...
void zc_app::chat_area::initialize(const float start_pos_x, data_manager *data_mgr)
{
    scene_width = 960;
    scene_height = 540;
...
void zc_app::chat_area::clear_chat(const std::string &system_message)
{
...
void zc_app::chat_area::switch_conversation(const std::string &conversation_partner_name)
{
...
void zc_app::chat_area::scroll_to_bottom(bool instant)
{
    if (total_messages_height > visible_messages_height)
    {
...
void zc_app::chat_area::add_message(const std::string &content, const bool is_sent, const bool auto_scroll)
{
...
const std::string &zc_app::chat_area::get_current_conversation_id() const
{
    return current_conversation_id;
}

void zc_app::chat_area::remove_conversation(const std::string &id)
{
...
void zc_app::chat_area::send_current_message()
{
...
void zc_app::chat_area::draw_glassy()
{
    chat_area_glass.draw();
    input_button.draw_glassy();
}

void zc_app::chat_area::on_mouse_down(const zcg_mouse_pos_t mouse_pos, const int button)
{
...
void zc_app::chat_area::on_mouse_up(const zcg_mouse_pos_t mouse_pos, const int button)
{
    if (button == ZCG_MOUSE_BUTTON_LEFT)
    {
...
void zc_app::chat_area::on_char_typed(unsigned int char_code)
{
...
void zc_app::chat_area::on_key_down(const zcg_key_event_t key_event)
{
...
void zc_app::chat_area::on_mouse_move(const zcg_mouse_pos_t &mouse_pos)
{
...
void zc_app::chat_area::on_scroll(const zcg_scroll_event_t &scroll_event)
{
...
std::vector<std::string> zc_app::chat_area::get_conversation_ids() const
{
    if (!p_data_manager)
    {
        return {};
...
const zc_app::conversation_data *zc_app::chat_area::get_conversation(const std::string &id) const
{
    if (!p_data_manager)
    {
        return nullptr;
