...
namespace zc_app
{
    void connection_modal::set_error_message(const std::string &message)
    {
        error_text.set_text(message);

        if (!message.empty())
        {
            const auto length = static_cast<float>(message.length());

            constexpr float error_char_height = SDF_BASE_SIZE * 0.05f;
            const float estimated_error_width = length * error_char_height * FONT_ASPECT_RATIO;
    ...
    void connection_modal::execute_connect()
    {
        if (is_connecting)
        {
            return;
        ...
        const float button_x = connect_button.get_container().get_x();
        const float button_char_height = SDF_BASE_SIZE * connect_button_text.get_style().text_size_magnification;
        const float estimated_width = connect_button_text.get_text().length() * button_char_height * FONT_ASPECT_RATIO;
        const float new_text_x = button_x + (BUTTON_WIDTH - estimated_width) / 2.0f;
        connect_button_text.set_x(new_text_x);
    ...
    void connection_modal::on_connection_result(const bool success, const std::string &message)
    {
        is_connecting = false;
        ...
        else
        {
            ...
            connect_button_text.set_x(original_button_text_x_);
    ...
    std::string connection_modal::get_username() const
    {
        return username_input.get_text();
    }

    void connection_modal::initialize(int scene_w, int scene_h)
    {
        ...
        constexpr float input_width = MODAL_WIDTH - PADDING * 4;
        ...
        container input_container(input_start_x, current_y, input_width, INPUT_HEIGHT);

        username_input.initialize(input_container, "Username (Optional)", &font_manager::get_font("Roboto-Regular"),
                                  input_common_style);
        username_input.set_char_validator([](unsigned int c) { return std::isalnum(c) || c == '_' || c == '-'; });

        current_y += INPUT_HEIGHT + PADDING;
        input_container.set_y(current_y);

        ip_address_input.initialize(input_container, "IP Address", &font_manager::get_font("Roboto-Regular"),
                                    input_common_style);
        ...
        port_input.initialize(input_container, "Port", &font_manager::get_font("Roboto-Regular"), input_common_style);
        ...
        original_button_text_x_ = btn_text_x;
    ...
    void connection_modal::set_visible(bool visible)
    {
        is_visible = visible;
        if (is_visible)
        {
    ...
    bool connection_modal::is_modal_visible() const
    {
        return is_visible;
    }

    void connection_modal::set_on_connect_callback(
        std::function<void(const std::string &, const std::string &, const std::string &)> callback)
    {
        on_connect_callback = std::move(callback);
    }

    void connection_modal::update(float delta_time)
    {
        if (!is_visible)
        {
            return;
    ...
    void connection_modal::on_char_typed(unsigned int char_code)
    {
        if (!is_visible || is_connecting)
        {
            return;
        }
    ...
    void connection_modal::on_mouse_down(const zcg_mouse_pos_t &mouse_pos, int button)
    {
    ...
    void connection_modal::on_key_down(const zcg_key_event_t &key_event)
    {
        ...
        if (username_input.is_active_field())
        {
            username_input.handle_key_down(key_event);
        }
        else if (ip_address_input.is_active_field())
        {
            ip_address_input.handle_key_down(key_event);
        }
        else if (port_input.is_active_field())
        {
            port_input.handle_key_down(key_event);
        }
    }

    void connection_modal::on_mouse_move(const zcg_mouse_pos_t &mouse_pos)
    {
        if (!is_visible)
        {
            return;
