...
void zc_app::main_scene::initialize(const int scene_width, const int scene_height)
{
    ...
    const container sidebar_container(PADDING, PADDING, sidebar_width - PADDING,
                                      static_cast<float>(scene_height) - PADDING * 2);

    const container profile_container(PADDING * 2, PADDING * 2, sidebar_width - PADDING * 4, 70.0f);

    const container avatar_container(profile_container.get_x() + PADDING * 2,
                                     profile_container.get_y() + (profile_container.
                                         get_height() - PADDING) / 2.0f, 40.0f, 40.0f);

    const container search_container(PADDING * 3, profile_container.get_y() + 100, sidebar_width - PADDING * 6, 1.0f);

    const container connections_container(PADDING * 2, search_container.get_y() + PADDING,
                                          sidebar_width - PADDING * 4, 30.0f);
...
void zc_app::main_scene::setup_sidebar_text_styles()
{
...
void zc_app::main_scene::setup_bridge_callbacks()
{
...
void zc_app::main_scene::draw_components()
{
...
void zc_app::main_scene::resize(const int width, const int height)
{
    scene_width = width;
    scene_height = height;
...
void zc_app::main_scene::destroy()
{
}

void zc_app::main_scene::process_main_thread_actions()
{
...
void zc_app::main_scene::queue_main_thread_action(std::function<void()> action)
{
    std::lock_guard lock(main_thread_action_mutex);
    main_thread_actions.push_back(std::move(action));
}

void zc_app::main_scene::on_char_typed(unsigned int char_code)
{
...
void zc_app::main_scene::on_mouse_down(const zcg_mouse_pos_t mouse_pos, const int button)
{
    if (connection_modal_widget.is_modal_visible())
    {
    ...
    else
    {
        ...
        if (button == ZCG_MOUSE_BUTTON_LEFT && zc_app::is_in_area(user_avatar_texture.get_container(), mouse_pos))
        {
            connection_modal_widget.set_visible(true);
        }
    }
...
void zc_app::main_scene::on_key_up(zcg_key_event_t key_event)
{
}

void zc_app::main_scene::on_mouse_up(const zcg_mouse_pos_t mouse_pos, const int button)
{
    if (connection_modal_widget.is_modal_visible() || request_modal_widget.is_modal_visible())
    {
    }
...
void zc_app::main_scene::on_key_down(const zcg_key_event_t key_event)
{
    if (connection_modal_widget.is_modal_visible())
    {
...
void zc_app::main_scene::scroll(const zcg_scroll_event_t &scroll_event)
{
    if (connection_modal_widget.is_modal_visible() || request_modal_widget.is_modal_visible())
    {
    }
...
void zc_app::main_scene::on_mouse_move(const zcg_mouse_pos_t &mouse_pos)
{
    if (connection_modal_widget.is_modal_visible() || request_modal_widget.is_modal_visible())
    {
        if (connection_modal_widget.is_modal_visible())
