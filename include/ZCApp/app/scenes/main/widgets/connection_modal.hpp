...
public:
    ...
    [[nodiscard]] std::string get_username() const { return username_input.get_text(); }
    ...
    void initialize(int scene_w, int scene_h)
    {
        ...
        username_input.initialize(input_start_x, current_y, input_width, INPUT_HEIGHT, "Username (Optional)", &font_manager::get_font("Roboto-Regular"), input_common_style);
        username_input.set_char_validator([](unsigned int c) { return std::isalnum(c) || c == '_' || c == '-'; });

        current_y += INPUT_HEIGHT + PADDING;
        ip_address_input.initialize(input_start_x, current_y, input_width, INPUT_HEIGHT, "IP Address", &font_manager::get_font("Roboto-Regular"), input_common_style);
        ip_address_input.set_char_validator([](unsigned int c) { return std::isdigit(c) || c == '.'; });

        current_y += INPUT_HEIGHT + PADDING;
        port_input.initialize(input_start_x, current_y, input_width, INPUT_HEIGHT, "Port", &font_manager::get_font("Roboto-Regular"), input_common_style);
