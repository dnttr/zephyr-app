...
namespace zc_app
{
    namespace validation
    {
        ...
        inline bool is_valid_port(const std::string& port_str) {
            if (!std::all_of(port_str.begin(), port_str.end(), ::isdigit) || port_str.empty()) {
                return false;
            }
            try {
    ...
    private:
        ...
        rectangle overlay_background{colour(0, 0, 0, 150), colour(0, 0, 0, 0), 0, 0.0F};
        rectangle modal_panel{dark_glass, glass_border, 1, BORDER_RADIUS};
        ...
        void execute_connect()
        {
            ...
            if (!username.empty() && !validation::is_valid_username(username)) {
                error_text.set_text("Invalid username (use A-Z, 0-9, _, -)");
                return;
            }
            if (!validation::is_valid_ip_address(ip)) {
                error_text.set_text("Invalid IP address format");
                return;
            }
            if (!validation::is_valid_port(port)) {
                error_text.set_text("Port must be a number (1-65535)");
                return;
            }

            error_text.set_text("Connecting...");
            is_connecting = true;
    ...
    public:
        ...
        void initialize(int scene_w, int scene_h)
        {
            ...
            title_style.text_size_magnification = 0.08F;
            ...
            const float title_y = modal_y + PADDING * 2;
            ...
            const float button_y = modal_y + MODAL_HEIGHT - BUTTON_HEIGHT - PADDING * 4;
            ...
            button_text_style.text_size_magnification = 0.06F;
            ...
            error_style.text_color = colour(255, 80, 80, 255);
            error_style.shadow_enable = true;

            error_text.initialize("", {0,0}, font_manager::get_font("Roboto-Regular"), error_style);

            const float error_y = button_y + BUTTON_HEIGHT + PADDING;
            error_text.set_properties_position(modal_x + PADDING * 2, error_y);
        }
        ...
        void set_visible(bool visible)
        {
            ...
            else
            {
                ...
                ip_address_input.clear_text();
                port_input.clear_text();
        ...
        void draw()
        {
            ...
            const std::string current_error = error_text.get_text();
            if (!current_error.empty()) {
                const float error_char_height = SDF_BASE_SIZE * 0.05f;
                const float estimated_error_width = current_error.length() * error_char_height * FONT_ASPECT_RATIO;
            ...
            connect_button.set_colour(is_button_hovered ? button_hover_color : button_original_color);
            connect_button.draw();
        ...
        void on_mouse_down(const zcg_mouse_pos_t &mouse_pos, int button)
        {
            ...
            if (!is_in_area(modal_panel.get_container(), mouse_pos))
            {
                set_visible(false);
                return;
            }
        ...
        void on_key_down(const zcg_key_event_t &key_event)
        {
            ...
            if (key_event.key_code == ZCG_KEY_ESCAPE)
            {
                set_visible(false);
                return;
            }
        ...
        void on_mouse_move(const zcg_mouse_pos_t &mouse_pos)
        {
            ...
            is_button_hovered = is_in_area(connect_button.get_container(), mouse_pos);
        }
    };
}